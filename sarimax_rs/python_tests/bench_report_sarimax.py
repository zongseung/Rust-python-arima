"""
bench_report_sarimax.py — SARIMAX vs statsmodels 종합 비교 리포트

측정 항목:
  - 수렴률, 우도(loglike), AIC, BIC
  - 파라미터 유사도 (MAE vs statsmodels) [서브셋 p,q,P,Q ≤ 2만]
  - 우도 유사도 (Δloglike)               [서브셋만]
  - 속도 비교 (sarimax_rs vs statsmodels)
  - h=6 예측 RMSE 비교

범위:
  - 전체 매트릭스: p,q,P,Q ∈ [0..5], d=1, D=1 per s=7,12,24, k_exog ∈ {1,2}
  - statsmodels 비교 서브셋: p,q,P,Q ∈ [0..2], k_exog=1

출력: ver5/result_v1/sarimax_report.md
"""
...
OUT = Path(__file__).resolve().parent.parent.parent / "ver5" / "result_v1" / "sarimax_report.md"
...
SM_TOL_PARAMS   = 0.10
SM_TOL_LOGLIKE  = 5.0
SM_SUBSET_MAX   = 2     # statsmodels 비교 서브셋: p,q,P,Q ≤ 2
SM_KEXOG        = 1     # statsmodels 비교는 k_exog=1만
...
def gen_sarimax_data(n, s, k_exog=1, seed=SEED):
    """Seasonal ARIMA(1,1,1)(1,1,1)_s + exogenous DGP."""
    rng  = np.random.default_rng(seed)
    exog = rng.normal(size=(n, k_exog))
    betas = rng.uniform(0.5, 1.5, size=k_exog)
    y = np.zeros(n + s + 2)
...
def run_rs(y, exog, order, seasonal):
    t0 = time.perf_counter()
    try:
        r = sarimax_rs.sarimax_fit(y, order, seasonal, exog=exog)
    except Exception:
...
def run_sm(y, exog, order, seasonal):
    if not HAS_SM:
        return None, None
    try:
        t0  = time.perf_counter()
        m   = SM_SARIMAX(y, exog=exog, order=order, seasonal_order=seasonal,
...
def forecast_rmse(y, exog, order, seasonal, h=STEPS):
    origins = [int(len(y)*0.7), int(len(y)*0.8), int(len(y)*0.9)]
    errs = []
    for orig in origins:
        ty = y[:orig]; te = exog[:orig]
        ay = y[orig:orig+h]; fe = exog[orig:orig+h]
        if len(ay) < h:
            continue
        r2 = sarimax_rs.sarimax_fit(ty, order, seasonal, exog=te)
        if not r2["converged"]:
            continue
        fc = sarimax_rs.sarimax_forecast(
            ty, order, seasonal, np.array(r2["params"]), steps=h,
            exog=te, future_exog=fe
        )
        errs.extend((np.array(fc["mean"]) - ay)**2)
    return math.sqrt(np.mean(errs)) if errs else float("nan")


# ─── 주기별 섹션 생성 ─────────────────────────────────────────────────────────

def bench_season_kexog(s, k_exog):
    n_obs  = N_OBS_MAP[s]
    y, exog = gen_sarimax_data(n_obs, s, k_exog)
    ...
    combos_sm   = {(p, 1, q, P, 1, Q, s)
                   ...
                   } if k_exog == SM_KEXOG else set()

    rows = []
    total = len(combos_full)
    print(f"  s={s} k_exog={k_exog}: {total} combos...", flush=True)
    ...
    for idx, combo in enumerate(combos_full):
        ...
        rs_r, rs_t = run_rs(y, exog, order, seasonal)
        ...
        if combo in combos_sm:
            sm_r, sm_t2 = run_sm(y, exog, order, seasonal)
            if sm_r is not None:
                try:
                    ll_sm    = sm_r.llf
                    aic_sm   = sm_r.aic
                    ...
                    if n_p > 0 and conv and len(rs_r["params"]) >= n_p:
                        rs_flat = np.array(rs_r["params"])[:n_p]
                        sm_pq   = sm_r.params[:p+q+P+Q]
                        sm_exog = sm_r.params[p+q+P+Q:p+q+P+Q+k_exog]
                        sm_flat = np.concatenate([sm_exog, sm_pq])
                        mae_par = float(np.mean(np.abs(rs_flat - sm_flat)))
                except Exception:
                    pass
        ...
        rows.append({
            "order": order, "seasonal": seasonal, "k_exog": k_exog, "conv": conv,
...
def section_for_season_kexog(s, k_exog, rows, y, exog):
    ...
    rmse_rows = []
    for r in best5:
        rmse = forecast_rmse(y, exog, r["order"], r["seasonal"])
    ...
    lines = []
    lines += [
        f"## s={s}, k_exog={k_exog} 섹션 (n={N_OBS_MAP[s]})\n",
        "### 수렴률\n",
        "| 항목 | 값 |",
        "|------|-----|",
        f"| 전체 조합 | {n_total} |",
    ...
    if rs_times and sm_times:
        ...
        lines.append(f"\nsarimax_rs는 statsmodels 대비 **{spd:.1f}x** 빠름 (p50 기준, subset)\n")
    lines.append("")

    lines += [
        f"### statsmodels 유사도 (서브셋: p,q,P,Q ≤ {SM_SUBSET_MAX}, k_exog={SM_KEXOG})\n",
        "| 지표 | 평균 | 중앙값 | p95 | 합격률 |",
        "|------|-----:|------:|----:|------:|",
    ]
    if delta_lls:
    ...
    lines += [
        "### Top-20 모델 (AIC 기준)\n",
        "| Rank | Order | Seasonal | k_exog | loglike | AIC(rs) | AIC(sm) | ΔAIC | Param MAE | Speed(ms) |",
        "|-----:|-------|---------|-------:|--------:|--------:|--------:|-----:|----------:|----------:|",
    ]
    for i, r in enumerate(sorted_rows[:20]):
        ...
        lines.append(
            f"| {i+1} | ARIMA{r['order']} | {r['seasonal']} | {k_exog} | {r['ll_rs']:.2f} | {r['aic_rs']:.1f} | {aic_sm_s} | {daics} | {mae_s} | {r['spd_rs']:.2f} |"
        )
    lines.append("")

    if rmse_rows:
        lines += [
            f"### 예측 RMSE (Top-5, h={STEPS})\n",
            "| Order | Seasonal | RMSE |",
            "|-------|---------|-----:|",
        ]
    ...
    if sorted_rows:
        ...
        lines += [
            ...
            f"- 최적 모델 (AIC): **SARIMAX{best['order']}×{best['seasonal']}** (AIC={best['aic_rs']:.1f})",
        ]
        if delta_lls:
...
def main():
    all_lines = [
        ...
        f"> 전체 매트릭스: p,q ∈ [0..{MAX_P}], P,Q ∈ [0..{MAX_PP}], d=1, D=1, k_exog ∈ {{1,2}}  ",
        f"> statsmodels 서브셋: p,q,P,Q ∈ [0..{SM_SUBSET_MAX}], k_exog={SM_KEXOG}  ",
        "",
    ]
    ...
    for s in [7, 12, 24]:
        for k_exog in [1, 2]:
            print(f"\n[SARIMAX s={s} k_exog={k_exog}]", flush=True)
            rows, y, exog = bench_season_kexog(s, k_exog)
            section = section_for_season_kexog(s, k_exog, rows, y, exog)
            all_lines.extend(section)
            n_conv = sum(1 for r in rows if r["conv"])
            summaries.append((s, k_exog, n_conv, len(rows)))

    # 전체 요약
    all_lines += [
        "## 전체 요약\n",
        "| s | k_exog | 전체 조합 | 수렴 | 수렴률 |",
        "|---|-------:|----------:|-----:|------:|",
    ]
    for s, ke, nc, nt in summaries:
        all_lines.append(f"| {s} | {ke} | {nt} | {nc} | {nc/nt*100:.1f}% |")
    all_lines.append("")

    OUT.write_text("\n".join(all_lines), encoding="utf-8")
    print(f"\n✓ 저장: {OUT}")
    for s, ke, nc, nt in summaries:
        print(f"  s={s} k_exog={ke}: {nc}/{nt} ({nc/nt*100:.1f}%) 수렴")
