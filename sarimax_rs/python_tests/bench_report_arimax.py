"""
bench_report_arimax.py — ARIMAX vs statsmodels 종합 비교 리포트

측정 항목:
  - 수렴률, 우도(loglike), AIC, BIC
  - 파라미터 유사도 (MAE vs statsmodels)
  - 우도 유사도 (Δloglike)
  - 속도 비교 (sarimax_rs vs statsmodels)
  - h=6 예측 RMSE 비교

범위: p,q ∈ [0..5], d=1, exog_k ∈ {1, 2, 3} (36 × 3 = 108 조합)
출력: ver5/result_v1/arimax_report.md
"""

import math
import sys
import time
from datetime import datetime
from itertools import product
from pathlib import Path

import numpy as np

sys.path.insert(0, "python")
import sarimax_rs

try:
    from statsmodels.tsa.statespace.sarimax import SARIMAX as SM_SARIMAX
    HAS_SM = True
except ImportError:
    HAS_SM = False

OUT = Path(__file__).resolve().parent.parent.parent / "ver5" / "result_v1" / "arimax_report.md"
OUT.parent.mkdir(parents=True, exist_ok=True)

N_OBS  = 500
STEPS  = 6
SEED   = 42
SM_TOL_PARAMS  = 0.10
SM_TOL_LOGLIKE = 5.0


# ─── 데이터 생성 ──────────────────────────────────────────────────────────────

def gen_arimax_data(n=N_OBS, k_exog=1, seed=SEED):
    """ARIMA(1,1,1) + exogenous variables DGP."""
    rng   = np.random.default_rng(seed)
    exog  = rng.normal(size=(n, k_exog))
    betas = rng.uniform(0.5, 1.5, size=k_exog)
    y = np.zeros(n)
    e = rng.normal(size=n)
    for i in range(2, n):
        y[i] = (y[i-1]
                + 0.4 * (y[i-1] - y[i-2])
                - 0.3 * e[i-1]
                + float(exog[i] @ betas)
                + e[i])
    return y, exog


# ─── 단일 모델 실행 ───────────────────────────────────────────────────────────

def run_rs(y, exog, order):
    t0 = time.perf_counter()
    r  = sarimax_rs.sarimax_fit(y, order, (0, 0, 0, 0), exog=exog)
    return r, time.perf_counter() - t0


def run_sm(y, exog, order):
    if not HAS_SM:
        return None, None
    try:
        t0  = time.perf_counter()
        m   = SM_SARIMAX(y, exog=exog, order=order, seasonal_order=(0, 0, 0, 0),
                         enforce_stationarity=False, enforce_invertibility=False)
        res = m.fit(disp=False, maxiter=200)
        return res, time.perf_counter() - t0
    except Exception:
        return None, None


def forecast_rmse(y, exog, order, h=STEPS):
    origins = [int(len(y)*0.7), int(len(y)*0.8), int(len(y)*0.9)]
    errs = []
    for orig in origins:
        train_y    = y[:orig]
        train_exog = exog[:orig]
        actual     = y[orig:orig+h]
        fc_exog    = exog[orig:orig+h]
        if len(actual) < h:
            continue
        r2 = sarimax_rs.sarimax_fit(train_y, order, (0, 0, 0, 0), exog=train_exog)
        if not r2["converged"]:
            continue
        fc = sarimax_rs.sarimax_forecast(
            train_y, order, (0, 0, 0, 0), np.array(r2["params"]),
            steps=h, exog=train_exog, future_exog=fc_exog
        )
        errs.extend((np.array(fc["mean"]) - actual)**2)
    return math.sqrt(np.mean(errs)) if errs else float("nan")


# ─── 메인 ─────────────────────────────────────────────────────────────────────

def main():
    combos = [(p, 1, q) for p, q in product(range(6), range(6))]

    lines = [
        "# ARIMAX 종합 비교 리포트\n",
        f"> 생성: {datetime.now():%Y-%m-%d %H:%M}  ",
        f"> sarimax_rs v{sarimax_rs.version()}  ",
        f"> statsmodels: {'설치됨' if HAS_SM else '미설치'}  ",
        f"> 데이터: n={N_OBS}, ARIMA(1,1,1)+exog DGP  ",
        f"> 범위: p,q ∈ [0..5], d=1, exog_k ∈ {{1,2,3}}  ",
        "",
    ]

    for k_exog in [1, 2, 3]:
        print(f"[ARIMAX k_exog={k_exog}] Starting...", flush=True)
        y, exog = gen_arimax_data(k_exog=k_exog)

        rows = []
        for order in combos:
            p, d, q = order
            rs_r, rs_t = run_rs(y, exog, order)
            sm_r, sm_t = run_sm(y, exog, order)

            conv   = rs_r.get("converged", False)
            ll_rs  = rs_r.get("loglike", float("nan")) if conv else float("nan")
            aic_rs = rs_r.get("aic",     float("nan")) if conv else float("nan")
            bic_rs = rs_r.get("bic",     float("nan")) if conv else float("nan")

            ll_sm = aic_sm = bic_sm = mae_par = float("nan")
            sm_t_val = float("nan")

            if sm_r is not None:
                try:
                    ll_sm    = sm_r.llf
                    aic_sm   = sm_r.aic
                    bic_sm   = sm_r.bic
                    sm_t_val = sm_t * 1000
                    # exog coef + AR + MA params
                    n_p = k_exog + p + q
                    if n_p > 0 and conv and len(rs_r["params"]) >= n_p:
                        # sarimax_rs layout: [exog | ar | ma]
                        rs_flat = np.array(rs_r["params"])[:n_p]
                        # statsmodels layout: [ar | ma | exog]
                        sm_flat = np.concatenate([sm_r.params[p+q:p+q+k_exog],
                                                  sm_r.params[:p+q]])
                        mae_par = float(np.mean(np.abs(rs_flat - sm_flat)))
                except Exception:
                    pass

            delta_ll  = abs(ll_rs - ll_sm)  if (math.isfinite(ll_rs)  and math.isfinite(ll_sm))  else float("nan")
            delta_aic = abs(aic_rs - aic_sm) if (math.isfinite(aic_rs) and math.isfinite(aic_sm)) else float("nan")

            rows.append({
                "order": order, "k_exog": k_exog, "conv": conv,
                "ll_rs": ll_rs, "ll_sm": ll_sm, "delta_ll": delta_ll,
                "aic_rs": aic_rs, "aic_sm": aic_sm, "delta_aic": delta_aic,
                "bic_rs": bic_rs, "bic_sm": bic_sm,
                "mae_par": mae_par,
                "spd_rs": rs_t * 1000,
                "spd_sm": sm_t_val,
            })

        conv_rows   = [r for r in rows if r["conv"]]
        n_total     = len(rows)
        n_conv      = len(conv_rows)
        sorted_rows = sorted(conv_rows, key=lambda r: r["aic_rs"] if math.isfinite(r["aic_rs"]) else 1e18)
        best5       = sorted_rows[:5]

        # RMSE
        rmse_rows = []
        for r in best5:
            rmse = forecast_rmse(y, exog, r["order"])
            rmse_rows.append((r["order"], rmse))

        rs_times   = [r["spd_rs"]    for r in rows if math.isfinite(r["spd_rs"])]
        sm_times   = [r["spd_sm"]    for r in rows if math.isfinite(r["spd_sm"])]
        delta_lls  = [r["delta_ll"]  for r in conv_rows if math.isfinite(r["delta_ll"])]
        delta_aics = [r["delta_aic"] for r in conv_rows if math.isfinite(r["delta_aic"])]
        mae_pars   = [r["mae_par"]   for r in conv_rows if math.isfinite(r["mae_par"])]

        # ── 섹션 출력
        lines += [
            f"## k_exog={k_exog} 섹션\n",
            f"### {k_exog}-1. 수렴률\n",
            "| 항목 | 값 |",
            "|------|-----|",
            f"| 전체 조합 | {n_total} |",
            f"| 수렴 | {n_conv} ({n_conv/n_total*100:.1f}%) |",
            f"| 미수렴 | {n_total-n_conv} |",
            "",
        ]

        lines += [
            f"### {k_exog}-2. 속도 비교 (ms/model)\n",
            "| Engine | p50 | p95 | max |",
            "|--------|----:|----:|----:|",
        ]
        if rs_times:
            lines.append(f"| sarimax_rs  | {np.median(rs_times):.2f} | {np.percentile(rs_times,95):.2f} | {max(rs_times):.2f} |")
        if sm_times:
            lines.append(f"| statsmodels | {np.median(sm_times):.2f} | {np.percentile(sm_times,95):.2f} | {max(sm_times):.2f} |")
        if rs_times and sm_times:
            spd = np.median(sm_times) / np.median(rs_times)
            lines.append(f"\nsarimax_rs는 statsmodels 대비 **{spd:.1f}x** 빠름\n")
        lines.append("")

        lines += [
            f"### {k_exog}-3. statsmodels 유사도\n",
            "| 지표 | 평균 | 중앙값 | p95 | 합격률 |",
            "|------|-----:|------:|----:|------:|",
        ]
        if delta_lls:
            pr = sum(1 for v in delta_lls if v < SM_TOL_LOGLIKE) / len(delta_lls) * 100
            lines.append(f"| Δloglike  | {np.mean(delta_lls):.3f} | {np.median(delta_lls):.3f} | {np.percentile(delta_lls,95):.3f} | {pr:.1f}% |")
        if delta_aics:
            lines.append(f"| ΔAIC      | {np.mean(delta_aics):.3f} | {np.median(delta_aics):.3f} | {np.percentile(delta_aics,95):.3f} | - |")
        if mae_pars:
            pr_p = sum(1 for v in mae_pars if v < SM_TOL_PARAMS) / len(mae_pars) * 100
            lines.append(f"| Param MAE | {np.mean(mae_pars):.4f} | {np.median(mae_pars):.4f} | {np.percentile(mae_pars,95):.4f} | {pr_p:.1f}% |")
        lines.append("")

        lines += [
            f"### {k_exog}-4. Top-20 모델 (AIC 기준)\n",
            "| Rank | Order | k_exog | loglike | AIC(rs) | AIC(sm) | ΔAIC | Param MAE | Speed(ms) |",
            "|-----:|-------|-------:|--------:|--------:|--------:|-----:|----------:|----------:|",
        ]
        for i, r in enumerate(sorted_rows[:20]):
            aic_sm_s = f"{r['aic_sm']:.1f}" if math.isfinite(r['aic_sm']) else "-"
            daics    = f"{r['delta_aic']:.2f}" if math.isfinite(r['delta_aic']) else "-"
            mae_s    = f"{r['mae_par']:.4f}"   if math.isfinite(r['mae_par'])   else "-"
            lines.append(
                f"| {i+1} | ARIMA{r['order']} | {k_exog} | {r['ll_rs']:.2f} | {r['aic_rs']:.1f} | {aic_sm_s} | {daics} | {mae_s} | {r['spd_rs']:.2f} |"
            )
        lines.append("")

        if rmse_rows:
            lines += [
                f"### {k_exog}-5. 예측 RMSE (Top-5, h={STEPS})\n",
                "| Order | RMSE |",
                "|-------|-----:|",
            ]
            for order, rmse in rmse_rows:
                rmse_s = f"{rmse:.4f}" if math.isfinite(rmse) else "-"
                lines.append(f"| ARIMAX{order} | {rmse_s} |")
            lines.append("")

        if sorted_rows:
            best = sorted_rows[0]
            lines += [
                f"### {k_exog}-6. 결론\n",
                f"- 수렴률: **{n_conv/n_total*100:.1f}%**",
                f"- 최적 모델 (AIC): **ARIMAX{best['order']}** (AIC={best['aic_rs']:.1f})",
            ]
            if delta_lls:
                pl = sum(1 for v in delta_lls if v < SM_TOL_LOGLIKE)
                lines.append(f"- Δloglike < {SM_TOL_LOGLIKE} 합격: **{pl}/{len(delta_lls)}**")
            lines.append("")

        print(f"  k_exog={k_exog}: {n_conv}/{n_total} 수렴", flush=True)

    OUT.write_text("\n".join(lines), encoding="utf-8")
    print(f"\n✓ 저장: {OUT}")


if __name__ == "__main__":
    main()
