...
pub fn batch_fit(
    ...
    series
        .par_iter()
        .map(|endog| optimizer::fit(endog, config, None, Some(method_str), Some(maxiter_val)))
        .collect()
}
...
mod tests {
    ...
    fn test_batch_fit_single() {
        ...
        let batch_results = batch_fit(&[data.clone()], &config, Some("lbfgs"), Some(500));
        assert_eq!(batch_results.len(), 1);
        let batch_result = batch_results[0].as_ref().unwrap();

        // Single series via direct fit
        let direct_result =
            optimizer::fit(&data, &config, None, Some("lbfgs"), Some(500)).unwrap();
    ...
    fn test_batch_loglike_matches_single() {
        ...
        let batch_ll = batch_loglike(&series, &config, &params);
    ...
    fn test_batch_forecast_matches_single() {
        ...
        let direct = forecast_pipeline(&data, &config, &params, 5, 0.05).unwrap();

        // Batch forecast
        let series = vec![data.clone(), data.clone()];
        let flat_params = params_vec.clone();
        let params_list = vec![flat_params.clone(), flat_params];
        let batch = batch_forecast(&series, &config, &params_list, 5, 0.05);
    ...
    fn test_batch_empty() {
        ...
        let results = batch_fit(&empty, &config, None, None);
