...
실행:
    cd rustima/
    .venv/bin/python python_tests/bench_sarima_scaling.py
"""
import json
import os
import signal
...
WORKER = os.path.join(PROJECT_ROOT, "python_tests", "bench_sarima_worker.py")
OUTDIR = os.path.join(PROJECT_ROOT, "..", "paper", "bench_2019_2023")
os.makedirs(OUTDIR, exist_ok=True)

# Hard guards
SWAP_DELTA_KILL_MB = 500          # swap delta vs baseline ≥ this → SIGKILL
HARD_TIMEOUT_S_RUSTIMA = 1800     # 30 min — rustima는 5y 322s가 측정치니 넉넉
HARD_TIMEOUT_S_PMDARIMA = 1800    # pmdarima도 동일. swap 가드가 더 빨리 잡음
...
def main():
    ...
    for years in (1, 2, 3, 4, 5):
        ...
        if p.status == "KILLED_SWAP":
            ...
            remaining = [y for y in (1, 2, 3, 4, 5) if y > years]
            for ry in remaining:
    ...
    for r in all_runs:
        ...
        rows.append(d)
    csv_path = os.path.join(OUTDIR, "sarima_scaling_results.csv")
...
if __name__ == "__main__":
    sys.path.insert(0, os.path.join(PROJECT_ROOT, "python_tests"))
    main()
